...
const getCrumbs = (pathname) => {
  if (!pathname || pathname === '/') {
    return [{ label: <HomeIcon/> }]
  }

  const segments = pathname.split('/').filter(Boolean)
  const crumbs = [{ label: <HomeIcon/>, to: '/' }]
  ...
  if (segments[0] === 'course' && segments[1]) {
    ...
    if (section) {
      crumbs.push({
        ...
        to:
          section === 'assignments' && segments[3]
            ? `/course/${courseId}/assignments`
            : section === 'grades' && segments[3]
    ...
    if (section === 'assignments' && segments[3]) {
      crumbs.push({
        ...
        to:
          segments[4] === 'tests' && segments[5] === 'build'
            ? `/course/${courseId}/assignments/${segments[3]}`
            : segments[4] === 'submissions' && segments[5]
              ? `/course/${courseId}/assignments/${segments[3]}?tab=Submissions`
              : undefined,
      })
    }

    if (section === 'assignments' && segments[4] === 'tests' && segments[5] === 'build') {
      crumbs.push({ label: 'Build Tests' })
    }

    if (section === 'assignments' && segments[4] === 'submissions' && segments[5]) {
      crumbs.push({ label: 'Submission Review' })
    }

    if (section === 'grades' && segments[3] === 'student' && segments[4]) {
      crumbs.push({ label: 'Student Gradebook' })
    }
