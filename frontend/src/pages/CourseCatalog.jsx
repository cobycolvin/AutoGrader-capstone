...
const TERM_SEASONS = ['Spring', 'Summer', 'Fall', 'Winter']

const buildTermOptions = () => {
...
function CourseCatalogPage({ user }) {
  ...
  const [termOptions] = useState(() => buildTermOptions())

  const canManageCourses = Boolean(user?.is_superuser || user?.is_instructor)
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setEnrolledIds(new Set((Array.isArray(myCourses) ? myCourses : []).map((course) => course.id)))
    } catch (err) {
  ...
  const columns = useMemo(
    ...
    [canManageCourses, enrolledIds, saving],
  )

  const normalizedTerm = form.term?.trim() || ''
  const isLegacyTerm = Boolean(normalizedTerm) && !termOptions.includes(normalizedTerm)
        ...
        >
          <Stack spacing={0.35}>
            ...
            <Typography variant="body2" color="text.secondary">
              Browse, enroll, and manage courses in one place.
            </Typography>
          </Stack>
          {canManageCourses ? (
          ...
          <Stack component="form" spacing={2} sx={{ mt: 1 }} onSubmit={handleSave}>
            ...
            <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
              ...
              >
                ...
                {isLegacyTerm ? <MenuItem value={normalizedTerm}>{`${normalizedTerm} (existing)`}</MenuItem> : null}
